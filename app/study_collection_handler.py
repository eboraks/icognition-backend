...
async def create_study_collection(
    ...
    with Session(engine) as session:
        ...
        for doc_id in documents_ids:
            doc = getter.get_document_by_id(doc_id)
            if doc:
                link_collection_document(collection.id, doc_id)
...
def get_study_collection_by_id(collection_id: str) -> StudyCollectionPublic:
    with Session(engine) as session:
        collection = session.scalar(
            select(Study_Collection)
            .options(joinedload(Study_Collection.tasks))
            .where(Study_Collection.id == collection_id)
        )
...
def get_study_collection_by_name(collection_name: str) -> StudyCollectionPublic:
    ...
    with Session(engine) as session:
        collection = session.scalar(
            select(Study_Collection)
            .options(joinedload(Study_Collection.tasks))
            .where(Study_Collection.name == collection_name)
        )
        return collection.to_public()


async def update_study_collection(
    collection: StudyCollectionPublic,
) -> StudyCollectionPublic:
    with Session(engine) as session:
        proj_exist = session.scalar(
            select(Study_Collection).where(Study_Collection.id == collection.id)
        )
        if proj_exist:
...
def get_study_collections_public(user_id: str) -> list[StudyCollectionPublic]:
    ...
    with Session(engine) as session:
        collections = session.scalars(
            select(Study_Collection).where(Study_Collection.user_id == user_id)
        ).all()
...
def delete_study_collection(collection_id: int) -> bool:
    ...
    with Session(engine) as session:
        collection = session.scalar(
            select(Study_Collection).where(Study_Collection.id == collection_id)
        )
        ...
        tasks = session.scalars(
            select(Study_Task).where(Study_Task.collection_id == collection_id)
        ).all()
        if collection:
            ...
            for task in tasks:
                session.execute(
                    delete(Study_Task_Citation).where(
                        Study_Task_Citation.task_id == task.id
                    )
                )
                session.delete(task)
...
def get_study_task(task_id: int) -> StudyTaskPublic:
    with Session(engine) as session:
        task = session.scalar(
            select(Study_Task)
            .options(joinedload(Study_Task.citations))
            .where(Study_Task.id == task_id)
        )
        return task.to_public()
    ...
    with Session(engine) as session:
        tasks = (
            session.scalars(
                select(Study_Task)
                .options(joinedload(Study_Task.citations))
                .where(Study_Task.collection_id == collection_id)
            )
            .unique()
            .all()
        )
...
async def update_study_task(task: StudyTaskPublic) -> StudyTaskPublic:
    ...
    try:
        ...
        if len(docs) == 0:
            logging.warning(
                f"No related documents found for collection_id: {_task.collection_id}"
            )
            with Session(engine) as session:
                session.add(_task)
...
def find_related_docs(
    ...
    with Session(engine) as session:
        stmt = (
            select(Study_Collection.objective_tasks_vector)
            .where(Study_Collection.id == collection_id)
            .scalar_subquery()
        )
        docs = session.scalars(
            select(Document).filter(
                Document.ai_summary_vector.cosine_distance(stmt)
                <= cosine_distance_freshhold
            )
        ).all()
        linked_docs = session.scalars(
            select(Document)
            .join(Study_Collection_Document_Link)
            .where(Study_Collection_Document_Link.collection_id == collection_id)
        ).all()
...
async def generate_collection_response(
    collection_id: str, listener: any = None
) -> None:
    ...
    if len(documents) > 0:
        ...
        logging.info(
            f"Generate collection's tasks responses. Found {len(documents)} related documents for collection_id: {collection_id}"
        )
        for task in collection.tasks:
            await generate_task_response(task, documents)
            logging.info(
                f"Generated response for collection_id: {collection_id} task_id: {task.id}"
            )

        if listener:
            listener(
                f"Generated responses completed for collection_id: {collection_id}"
            )
        logging.info(
            f"Generated responses completed for collection_id: {collection_id}"
        )
        ...
        with Session(engine) as session:
            ...
            for k, t in enumerate(types):
                ...
                for e_name in t.ents_names:
                    ent_node = session.scalar(
                        select(Entity).where(Entity.name == e_name)
                    ).to_node()
...
async def generate_task_response(
    task: Study_Task, documents: list[Document]
) -> StudyTaskPublic:
    ...
    try:
        response = await client.generate_response(
            AskQuestionPrompt.build_prompt(question=task.description, docs=documents),
            AskQuestionPrompt,
        )
        ...
        raise Exception(
            f"Error while generate response for task_id: {task.id}. Error: {e}"
        )


def insert_task_citations(
    task_public: StudyTaskPublic, response: AskQuestionPrompt
) -> StudyTaskPublic:
    ...
    with Session(engine) as session:
        ...
        if response.meta_answer == "SUCCESS":
            logging.info(
                f"Inserting citations for task_id: {task.id}. Number of citations: {len(response.documents_citations)}"
            )
            ...
            for doc_citation in response.documents_citations:
                study_task_citation = Study_Task_Citation(
                    task_id=task.id,
...
def delete_task_citations(task_id: int) -> int:
    with Session(engine) as session:
        results = session.execute(
            delete(Study_Task_Citation).where(Study_Task_Citation.task_id == task_id)
        )
        session.commit()
        return results.rowcount


def link_collection_document(
    ...
    with Session(engine) as session:
        ...
        session.commit()
        link = session.scalar(
            select(Study_Collection_Document_Link).where(
                Study_Collection_Document_Link.collection_id == collection_id
                and Study_Collection_Document_Link.document_id == document_id
            )
        )
...
def unlink_collection_document(collection_id: str, document_id: str) -> bool:
    ...
    with Session(engine) as session:
        session.execute(
            delete(Study_Collection_Document_Link).where(
                Study_Collection_Document_Link.collection_id == collection_id
                and Study_Collection_Document_Link.document_id == document_id
            )
        )
...
def get_collection_status(collection_id: str) -> str:
    ...
    number_of_tasks_with_response = len(
        [task for task in collection.tasks if task.status == "SUCCESS"]
    )
    number_of_tasks_with_ai_response = len(
        [task for task in collection.tasks if task.ai_explanation is not None]
    )
    ...
    return {
        ...
        "percentage_tasks_with_response": (
            number_of_tasks_with_response / number_of_tasks
        )
        * 100,
    }
