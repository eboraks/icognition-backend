from fastapi import FastAPI, HTTPException, BackgroundTasks, status, Response, Depends
...
def get_current_user(
    ...
    try:
        print("Yeah")
        decoded_user = auth.verify_id_token(credential.credentials)
        print("User ID: ", decoded_user["uid"])
        return decoded_user
    except Exception as e:
        print("Error updated: ", e)
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Unauthorized")
...
async def create_bookmark(
    payload: PagePayload, background_tasks: BackgroundTasks, response: Response, user=Depends(get_current_user)
):
...
async def post_regenerate_document(
    ...
    bookmark = app_logic.reassociate_bookmark_with_document(
...
async def generate_document(document_id):
    ...
    if document.status in ["Pending", "Done", "Failure"]:
        ...
        logging.info(f"Background task for document ID: {
...
async def regenerate_document(doc: Document):
    ...
    if doc.status in ["Pending", "Done", "Failure"]:
        ...
        if new_doc:
            logging.info(f"Background task for document ID: {
                         new_doc.id} completed")
        else:
            logging.error(
                f"Background document regenaring for document ID: {
                    doc.id} failed"
            )
...
async def get_document_plus(bookmark_id: int, response: Response, background_tasks: BackgroundTasks):
    ...
    if document.status in ["Processing", "Pending"]:
        response.status_code = status.HTTP_206_PARTIAL_CONTENT
        logging.info(
            f"Document plus -> endpoint called on document status {
                document.status}"
        )
...
async def post_entities(user_id: str):
...
async def search_documents(search_payload: SearchPayload):
    ...
    if (search_payload.query == None):
        documents = app_logic.search_documents(
            search_payload.user_id, search_payload.query)
    else:
        documents = app_logic.search_embeddings(
            search_payload.user_id, search_payload.query)
