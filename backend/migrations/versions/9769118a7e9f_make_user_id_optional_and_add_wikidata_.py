"""make_user_id_optional_and_add_wikidata_url

Revision ID: 9769118a7e9f
Revises: 005858a876b8
Create Date: 2026-01-08 10:58:18.780212
...
revision: str = '9769118a7e9f'
down_revision: Union[str, Sequence[str], None] = '005858a876b8'
...
def upgrade() -> None:
    ...
    op.drop_column('document', 'content_vector')
    op.add_column('entities', sa.Column('wikidata_url', sqlmodel.sql.sqltypes.AutoString(length=500), nullable=True))
    op.alter_column('entities', 'user_id',
               existing_type=sa.VARCHAR(),
               nullable=True)
    op.drop_column('entities', 'vector')
...
def downgrade() -> None:
    ...
    op.alter_column('prompts', 'content',
               existing_type=sa.TEXT(),
               nullable=False)
    op.add_column('entities', sa.Column('vector', pgvector.sqlalchemy.vector.VECTOR(dim=768), autoincrement=False, nullable=True))
    op.alter_column('entities', 'user_id',
               existing_type=sa.VARCHAR(),
               nullable=False)
    op.drop_column('entities', 'wikidata_url')
    op.add_column('document', sa.Column('content_vector', pgvector.sqlalchemy.vector.VECTOR(dim=1536), autoincrement=False, nullable=True))
