...
async def test_create_study_collection():
    ...
    collection = await handler.create_study_collection(
        name="test_collection",
        objective="test_objective",
        user_id="test_user",
        tasks_descriptions=["task1", "task2"],
    )

    collection = handler.get_study_collection_by_id(collection.id)
    assert collection.name == "test_collection"
    assert collection.description == "test_objective"
...
async def test_create_frech_revolution_collection():
    ...
    collection = await handler.create_study_collection(
        ...
        tasks_descriptions=tasks,
    )

    collection = handler.get_study_collection_by_id(collection.id)

    assert collection.name == name
    assert collection.description == objective
